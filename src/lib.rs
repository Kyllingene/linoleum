...
/// ```
#[derive(Debug, Clone)]
pub struct Editor<P: Display> {
    pub prompt: P,
    pub word_breaks: Box<str>,
...
impl<P: Display> Editor<P> {
    ...
    pub fn new(prompt: P) -> Self {
        Self {
            prompt,
            word_breaks: Box::from(WORD_BREAKS),
            highlight: None,
            history: None,
            completion: None,
        }
...
impl<P: Display> Editor<P> {
    ...
    ///     .word_breaks("".into());
    /// ```
    pub fn word_breaks(self, word_breaks: Box<str>) -> Editor<P> {
        Editor {
            word_breaks,
            ..self
        }
